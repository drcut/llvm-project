///===- FuncSimpleLoopUnswitch.cpp - Hoist loop-invariant control flow
///---------===//
//
...
#define DEBUG_TYPE "func-simple-loop-unswitch"
...
static cl::opt<bool> EnableNonTrivialUnswitch(
    "func-enable-nontrivial-unswitch", cl::init(false), cl::Hidden,
    cl::desc("Forcibly enables non-trivial loop unswitching rather than "
             "following the configuration passed into the pass."));

static cl::opt<int>
    UnswitchThreshold("func-unswitch-threshold", cl::init(50), cl::Hidden,
                      cl::ZeroOrMore,
                      cl::desc("The cost threshold for unswitching a loop."));

static cl::opt<bool> EnableUnswitchCostMultiplier(
    "func-enable-unswitch-cost-multiplier", cl::init(true), cl::Hidden,
...
static cl::opt<bool> DropNonTrivialImplicitNullChecks(
    "func-simple-loop-unswitch-drop-non-trivial-implicit-null-checks",
...
static cl::opt<unsigned>
    MSSAThreshold("func-simple-loop-unswitch-memoryssa-threshold",
...
static cl::opt<bool> FreezeLoopUnswitchCond(
    "func-freeze-loop-unswitch-cond", cl::init(true), cl::Hidden,
...
static void
deleteDeadBlocksFromLoop(Loop &L, SmallVectorImpl<BasicBlock *> &ExitBlocks,
                         DominatorTree &DT, LoopInfo &LI,
                         MemorySSAUpdater *MSSAU,
...
static bool rebuildLoopAfterUnswitch(Loop &L, ArrayRef<BasicBlock *> ExitBlocks,
  ...
  if (Blocks.empty()) {
    ...
    // typically done by using the UnswitchCB callback).
    LI.destroy(&L);
    return false;
  }
...
PreservedAnalyses FuncSimpleLoopUnswitchPass::run(Function &F,
  ...
  while (!Worklist.empty()) {
    ...
    Changed |= unswitchLoop(
        ...
        AM.getResult<AAManager>(F), AM.getResult<TargetIRAnalysis>(F), Trivial,
        NonTrivial, UnswitchCB, &AM.getResult<ScalarEvolutionAnalysis>(F),
...
void FuncSimpleLoopUnswitchPass::printPipeline(
    raw_ostream &OS, function_ref<StringRef(StringRef)> MapClassName2PassName) {
  static_cast<PassInfoMixin<FuncSimpleLoopUnswitchPass> *>(this)->printPipeline(
...
class FuncSimpleLoopUnswitchLegacyPass : public FunctionPass {
  bool NonTrivial;

public:
  static char ID; // Pass ID, replacement for typeid

  explicit FuncSimpleLoopUnswitchLegacyPass(bool NonTrivial = false)
      : FunctionPass(ID), NonTrivial(NonTrivial) {
    initializeFuncSimpleLoopUnswitchLegacyPassPass(
...
bool FuncSimpleLoopUnswitchLegacyPass::runOnFunction(Function &F) {
  if (skipFunction(F))
    return false;
  ...
  while (!Worklist.empty()) {
    ...
    auto UnswitchCB = [&L, &Worklist](bool CurrentLoopValid,
                                      bool PartiallyInvariant,
                                      ArrayRef<Loop *> NewLoops) {
      // If we did a non-trivial unswitch, we have added new (cloned) loops.
      for (auto *NewL : NewLoops)
        appendLoopsToWorklist(*NewL, Worklist);
      // LPM.addLoop(*NewL);
      ...
      if (CurrentLoopValid) {
        ...
        if (!PartiallyInvariant)
          // LPM.addLoop(*L);
          appendLoopsToWorklist(*L, Worklist);
      } else
        // LPM.markLoopAsDeleted(*L);
        Worklist.erase(L);
    };

    auto DestroyLoopCB = [&Worklist](Loop &L, StringRef /* Name */) {
      // LPM.markLoopAsDeleted(L);
      Worklist.erase(&L);
    };
    ...
    Changed |= unswitchLoop(*L, DT, LI, AC, AA, TTI, true, NonTrivial,
                            UnswitchCB, SE, &MSSAU, DestroyLoopCB);
  }
  if (VerifyMemorySSA)
    MSSA->verifyMemorySSA();
...
char FuncSimpleLoopUnswitchLegacyPass::ID = 0;
INITIALIZE_PASS_BEGIN(FuncSimpleLoopUnswitchLegacyPass,
                      "func-simple-loop-unswitch",
                      "Simple unswitch loops (function pass)", false, false)
INITIALIZE_PASS_DEPENDENCY(AssumptionCacheTracker)
INITIALIZE_PASS_DEPENDENCY(DominatorTreeWrapperPass)
INITIALIZE_PASS_DEPENDENCY(LoopInfoWrapperPass)
INITIALIZE_PASS_DEPENDENCY(MemorySSAWrapperPass)
INITIALIZE_PASS_DEPENDENCY(TargetTransformInfoWrapperPass)
INITIALIZE_PASS_END(FuncSimpleLoopUnswitchLegacyPass,
                    "func-simple-loop-unswitch", "Simple unswitch loops", false,
                    false)

Pass *llvm::createFuncSimpleLoopUnswitchLegacyPass(bool NonTrivial) {
  return new FuncSimpleLoopUnswitchLegacyPass(NonTrivial);
}